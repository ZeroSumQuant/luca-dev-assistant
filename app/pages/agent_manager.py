...
DEFAULT_AGENT_CONFIG = {
    ...
    "coder": {
        ...
        "model": "gpt-4",
        "available_models": ["gpt-4", "claude-3-opus", "gpt-3.5-turbo", "deepseek-coder"],
        "status": "idle",
        "color": "#4CAF50"
    },
    "tester": {
...
def main():
                ...
                with col2:
                    ...
                    if new_model != agent_info['model']:
                        if st.button("Update Model"):
                            st.session_state.agent_config[selected_agent]['model'] = new_model
                            st.success(f"Updated {agent_info['name']}'s model to {new_model}")
                            st.rerun()
    ...
    with tabs[2]:
        ...
        # Create status cards
        cols = st.columns(3)
        for i, (agent_id, agent_info) in enumerate(st.session_state.agent_config.items()):
            with cols[i % 3]:
                with st.container():
                    st.markdown(f"### {agent_info['name']}")
