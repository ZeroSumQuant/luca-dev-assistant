...
async def test_simple_request_processing():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        response_options = ResponseOptions(
            learning_mode=LearningMode.PRO, verbose=False, include_agent_info=False
        )
...
async def test_learning_modes():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        noob_options = ResponseOptions(
            learning_mode=LearningMode.NOOB, verbose=True, include_agent_info=True
        )
        ...
        guru_options = ResponseOptions(
            learning_mode=LearningMode.GURU, verbose=True, include_agent_info=False
        )
        ...
        assert all(
            len(resp) > 0 for resp in [noob_response, pro_response, guru_response]
        )
